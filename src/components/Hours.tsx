      ...
      {!day.isClosed && (
        <td className="pr-1">
          <span itemProp="openingHoursSpecification" itemScope itemType="http://schema.org/OpeningHoursSpecification">
            <span itemProp="opens">{convertTo12HourFormat(day.openIntervals[0].start, true)} -{" "}</span>
            <span itemProp="closes">{convertTo12HourFormat(day.openIntervals[0].end, true)}</span>
          </span>
        </td>
      )}
      {day.isClosed && (
...
const Hours = (props: HoursProps) => {
  ...
  return (
    <>
      <div>
        <h4 className="text-2xl font-bold text-gray-800 mb-3 border-b-2 border-yellow-300">{title}</h4>
        <table>
